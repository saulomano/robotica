...
export default class NoticiaComponent extends CuradorComponent {
	/*@ngInject*/
	constructor($scope, $element, $stateParams,$q, Auth, Restangular, $log, Util, $timeout, $state, $mdDialog, $mdConstant, ngMeta) {
		...
		this.PropuestaDesafio = this.Restangular.one('noticias', this.uid);
	...
	configureFunctions(){	
		...
		this.cancel = () => {
			(this.isEdit) ? this.$state.go('curador.dashboardnoticias') : this.deleteResource();
		};
		...
		this.toRefuse = ($event) => {
			...
			this.resource
					.put()	
					.then(data => {
						(this.returnDesafios) ? this.$state.go('curador.noticia', { type: "noticia" }) : this.$state.go('curador.propuestadesafio');
					})
					.catch(err => {
						throw err;
					});
	...
	getResource(){
		...
		.then(data => {
			...
			this.returnDesafios = (this.resource.type == 'noticia') ? true : false;
